...
func SimulateUpDownCounter(mp *sdkMetric.MeterProvider, conf *Config, logger *zap.Logger) {
	c := *conf
...
func upDownCounter(mp *sdkMetric.MeterProvider, c Config, logger *zap.Logger) WorkerFunc {
	return func(ctx context.Context) {
		...
		counter, _ := mp.Meter(c.ServiceName).Int64UpDownCounter(
			...
			apiMetric.WithDescription("UpDownCounter demonstrates how to measure numbers that can go up and down"),
		)
